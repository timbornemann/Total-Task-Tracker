...
export const requestLogger = (
  ...
  // Log request start
  console.log(
    `[${new Date().toISOString()}] ${correlationId} ${req.method} ${req.url} - Start`,
  );
...
export const securityErrorHandler = (
  ...
  console.error(
    `[${new Date().toISOString()}] ${correlationId} Security Error:`,
    err,
  );
...
export const inputSanitization = (
  ...
  const checkForMaliciousPatterns = (obj: unknown): boolean => {
    if (typeof obj === "string") {
      ...
      return [...sqlPatterns, ...xssPatterns].some((pattern) =>
        pattern.test(obj),
      );
    }
  ...
  if (
    checkForMaliciousPatterns(req.body) ||
