...
export const taskSelectors = {
  ...
  getMediumPriorityTasks: (tasks: Task[]) =>
    tasks.filter(task => task.priority === 'medium' && task.visible !== false),
  ...
  getRecentTasks: (tasks: Task[], days: number = 7) => {
    ...
    return tasks.filter(task => 
      task.createdAt >= cutoff && task.visible !== false
    );
  },
  ...
  getTaskStats: (tasks: Task[]) => {
    ...
    return {
      ...
      highPriority: visibleTasks.filter(task => task.priority === 'high').length,
...
export const noteSelectors = {
  ...
  getArchivedNotes: (notes: Note[]) =>
    notes.filter(note => note.archived),
  
  getActiveNotes: (notes: Note[]) =>
    notes.filter(note => !note.archived),
  
  // Sorted selectors
  getSortedNotes: (notes: Note[]) => {
    const pinned = notes.filter(note => note.pinned && !note.archived)
      .sort((a, b) => a.order - b.order);
    const unpinned = notes.filter(note => !note.pinned && !note.archived)
  ...
  // Template selectors
  getTemplateNotes: (notes: Note[]) =>
    notes.filter(note => note.isTemplate),
...
export const recurringTaskSelectors = {
  ...
  getRecurringTaskStats: (tasks: RecurringTask[]) => {
    ...
    return {
      ...
      monthly: tasks.filter(task => task.recurrence.type === 'monthly').length,
...
export const combinedSelectors = {
  ...
  getCategoryUsage: (tasks: Task[], notes: Note[], categories: Category[]) =>
    categories.map(category => ({
      ...
      totalItems: tasks.filter(task => task.categoryId === category.id).length +
  ...
  getGlobalStats: (
    ...
    recurringTasks: recurringTaskSelectors.getRecurringTaskStats(recurringTasks),
