...
export function useTasksStore(options: UseTasksStoreOptions = {}) {
  ...
  const updateTask = useCallback((taskId: string, updates: Partial<Task>) => {
    setTasks((prev) =>
      updateTaskRecursive(prev, taskId, (task) => ({
        ...task,
        ...updates,
        updatedAt: new Date(),
      })),
  ...
  const moveTaskToSubtask = useCallback(
    (taskId: string, newParentId: string) => {
      setTasks((prev) => {
  ...
  const reorderTasks = useCallback(
    (categoryId: string, startIndex: number, endIndex: number) => {
      setTasks((prev) => {
  ...
  const resetTask = useCallback((taskId: string) => {
    setTasks((prev) =>
      updateTaskRecursive(prev, taskId, (task) => resetTaskRecursive(task)),
    );
  }, []);
      ...
      prev.map((task) => {
        ...
        return {
          ...
          subtasks: task.subtasks.map((subtask) =>
            subtask.categoryId === categoryId
              ? resetTaskRecursive(subtask)
              : subtask,
          ),
        };
      }),
  ...
  const getTasksByFilters = useCallback(
    (customFilters?: TaskFilters): Task[] => {
      ...
      return tasks.filter((task) => {
        ...
        if (
          ...
          task.categoryId !== activeFilters.categoryId
        ) {
          return false;
        }
        ...
        if (activeFilters.dateRange) {
          ...
          if (
            taskDate < activeFilters.dateRange.start ||
...
function updateTaskRecursive(
  ...
  return tasks.map((task) => {
    ...
    if (task.subtasks.length > 0) {
      const updatedSubtasks = updateTaskRecursive(
        task.subtasks,
        taskId,
        updateFn,
      );
