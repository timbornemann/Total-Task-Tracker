...
export interface RecurringTask extends Omit<Task, 'id' | 'createdAt' | 'updatedAt'> {
  id: string;
...
export function useRecurringTasksStore(options: UseRecurringTasksStoreOptions = {}) {
  const {
  ...
  const shouldGenerateTask = useCallback((recurringTask: RecurringTask): boolean => {
  ...
  const addRecurringTask = useCallback((
    taskData: Omit<RecurringTask, 'id' | 'createdAt' | 'updatedAt' | 'nextGeneration'>
  ) => {
    const now = new Date();
  ...
  // Task generation
  const generateTaskFromRecurring = useCallback((recurringTask: RecurringTask): Task => {
    const now = new Date();
    ...
    const generatedTask: Task = {
      ...
      status: 'todo',
      completed: false,
      dueDate: recurringTask.dueDate ? new Date(recurringTask.dueDate) : undefined,
      createdAt: now,
      updatedAt: now,
      subtasks: [],
      pinned: false,
  ...
  const processRecurringTasks = useCallback(() => {
    ...
    return newTasks;
  }, [recurringTasks, shouldGenerateTask, generateTaskFromRecurring, calculateNextGeneration]);

  // Manual task generation
  const generateTaskNow = useCallback((recurringTaskId: string) => {
  ...
  // Query functions
  const getRecurringTaskById = useCallback((taskId: string): RecurringTask | undefined => {
  ...
  const getOverdueRecurringTasks = useCallback((): RecurringTask[] => {
    ...
    return recurringTasks.filter(task => 
      task.isActive && task.nextGeneration < now
    );
  }, [recurringTasks]);

  const getRecurringTasksByCategory = useCallback((categoryId: string): RecurringTask[] => {
    return recurringTasks.filter(task => task.categoryId === categoryId);
  }, [recurringTasks]);

  const getGeneratedTasksForRecurring = useCallback((recurringTaskId: string): Task[] => {
    return generatedTasks.filter(task => task.recurringTaskId === recurringTaskId);
  }, [generatedTasks]);
  ...
  const previewNextGenerations = useCallback((
    ...
    for (let i = 0; i < count; i++) {
      ...
      currentDate = calculateNextGeneration(recurringTask.recurrence, currentDate);
    }
  ...
  // Validation
  const validateRecurrencePattern = useCallback((pattern: RecurrencePattern): string | null => {
    ...
    switch (pattern.type) {
      case 'weekly':
        if (pattern.daysOfWeek && pattern.daysOfWeek.some(day => day < 0 || day > 6)) {
  ...
  return {
    ...
    startAutoProcessing: () => {
      if (!intervalRef.current && autoProcessInterval > 0) {
        intervalRef.current = setInterval(processRecurringTasks, autoProcessInterval);
      }
    },
    stopAutoProcessing: () => {
      if (intervalRef.current) {
