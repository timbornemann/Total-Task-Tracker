...
export function useNotesStore(options: UseNotesStoreOptions = {}) {
  ...
  const getFilteredNotes = useCallback(
    (customFilters?: NotesFilters): Note[] => {
      ...
      return notes.filter((note) => {
        ...
        if (
          ...
          note.categoryId !== activeFilters.categoryId
        ) {
          return false;
        }
        ...
        // Pinned filter
        if (
          activeFilters.pinned !== undefined &&
          note.pinned !== activeFilters.pinned
        ) {
          return false;
        }
        ...
        if (activeFilters.dateRange) {
          ...
          if (
            noteDate < activeFilters.dateRange.start ||
  ...
  const bulkUpdateNotes = useCallback(
    (noteIds: string[], updates: Partial<Note>) => {
      setNotes((prev) =>
        sortNotes(
          prev.map((note) =>
  ...
  const bulkDeleteNotes = useCallback(
    (noteIds: string[]) => {
      setNotes((prev) =>
        sortNotes(prev.filter((note) => !noteIds.includes(note.id))),
      );
    },
    [sortNotes],
  );
  ...
  const createTemplate = useCallback(
    (noteId: string, templateName: string) => {
