...
export function useApiWithNotifications(options: UseApiOptions = {}) {
  ...
  const makeApiCall = useCallback(
    async <T>(
      ...
      if (ongoingRequests.current.has(requestKey)) {
        console.warn(
          `[useApiWithNotifications] Duplicate request prevented: ${requestKey}`,
        );
        return null;
      }
  ...
  const get = useCallback(
    <T>(
      ...
      config?: ErrorNotificationConfig,
      requestConfig?: Omit<ApiRequestConfig, "method" | "body">,
    ) => makeApiCall<T>("get", url, undefined, context, config, requestConfig),
    [makeApiCall],
  );
  ...
  const post = useCallback(
    ...
    ) => makeApiCall<T>("post", url, body, context, config, requestConfig),
    [makeApiCall],
  );
  ...
  const put = useCallback(
    <T>(
      ...
      config?: ErrorNotificationConfig,
      requestConfig?: Omit<ApiRequestConfig, "method">,
    ) => makeApiCall<T>("put", url, body, context, config, requestConfig),
    [makeApiCall],
  );
  ...
  const del = useCallback(
    <T>(
      ...
      makeApiCall<T>("delete", url, undefined, context, config, requestConfig),
    [makeApiCall],
  );
  ...
  const patch = useCallback(
    <T>(
      ...
      config?: ErrorNotificationConfig,
      requestConfig?: Omit<ApiRequestConfig, "method">,
    ) => makeApiCall<T>("patch", url, body, context, config, requestConfig),
    [makeApiCall],
  );
   ...
   */
  const createResourceApi = useCallback(
    <T = unknown>(resourceName: string) => ({
      create: (data: T, context?: Partial<ErrorContext>) =>
        post<T>(`/api/${resourceName}`, data, {
          resource: resourceName,
          operation: "create",
          ...context,
        }),

      update: (id: string, data: Partial<T>, context?: Partial<ErrorContext>) =>
        put<T>(`/api/${resourceName}/${id}`, data, {
          resource: resourceName,
          operation: "update",
          ...context,
        }),

      delete: (id: string, context?: Partial<ErrorContext>) =>
        del(`/api/${resourceName}/${id}`, {
          resource: resourceName,
          operation: "delete",
          ...context,
        }),

      get: (id: string, context?: Partial<ErrorContext>) =>
        get<T>(`/api/${resourceName}/${id}`, {
          resource: resourceName,
          operation: "fetch",
          ...context,
        }),

      list: (context?: Partial<ErrorContext>) =>
        get<T[]>(`/api/${resourceName}`, {
          resource: resourceName,
          operation: "fetch",
          ...context,
        }),

      sync: (data: T[], context?: Partial<ErrorContext>) =>
        put<T[]>(`/api/${resourceName}`, data, {
          resource: resourceName,
          operation: "sync",
          ...context,
        }),
...
export function useSyncApi() {
  ...
  const fetchData = useCallback(
    async <T>(endpoint: string, context?: Partial<ErrorContext>) => {
      return api.get<T>(
        endpoint,
        {
          action: "Daten laden",
